	...
	templates: [{
		...
		outputs: artifacts: [{
			...
			gcs: {
				...
				key: """
				metrics/{{ workflow.creationTimestamp.Y }}/{{ workflow.creationTimestamp.m }}/{{ workflow.creationTimestamp.d }}/{{ workflow.name }}/{{ inputs.parameters.appLabel }}-{{ inputs.parameters.chaosType }}-{{ inputs.parameters.jobN }}
				"""
			}
		}]
	}, {
